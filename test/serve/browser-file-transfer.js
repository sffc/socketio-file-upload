...
function runTest(t, isWrapData) {
	var client;
	if (isWrapData) {
		client = new SiofuClient(socket, {
	...
	client.addEventListener("choose", function (ev) {
		...
		socket.emit(isWrapData ? "numSubmittedWrap" : "numSubmitted", numSubmitted);
...
test("Test wrap data and send to a single topic", function (t) {
	runTest(t, true);
});
